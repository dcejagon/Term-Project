...
\item Run NAME.\+text through \mbox{\hyperlink{GcodeInterpreter_8py}{Gcode\+Interpreter.\+py}} to output text files for desired R and Theta motor angles
\item Save all necessary files on the Nucleo
\item Run \mbox{\hyperlink{main_8py}{main.\+py}} on the Nucleo to use the plotter ~\newline
 a. \mbox{\hyperlink{main_8py}{main.\+py}} uses \mbox{\hyperlink{cotask_8py}{cotask.\+py}} for cooperative multitasking. ~\newline
...
These two files are very similar, the only differnece is that \mbox{\hyperlink{EncoderDriver_8py}{Encoder\+Driver.\+py}} reads the Theta encoder and Encoder.\+py reads only the R encoder. These files are only used to calculate encoder position. When we read the encoder position, we write that value to a shared variable for each position. For Theta\+Encoder\+Position.\+write(\+Encoder\+Position). We calcualte this encoder position by calculating the encoder delta from a timer on the microcontroller and the previous time. This allows us to calculate encoder position without worrying about the encoder \char`\"{}overflowing,\char`\"{} where the encoder position essentially resets to 0 once the reading reaches 65535. We decided to split \mbox{\hyperlink{EncoderDriver_8py}{Encoder\+Driver.\+py}} and Encoder.\+py to make it easy to troubleshoot any errors that may come up. Origionally, \mbox{\hyperlink{EncoderDriver_8py}{Encoder\+Driver.\+py}} was responsible for reading both encoder positions, so that is why it has R encoder pins, timer, and shares passed into the class, but they are never used. Similarly, since we only need R motor stuff passed into Encoder, we changed that file to only have necessary values shared to that file. In short, instead of changing \mbox{\hyperlink{EncoderDriver_8py}{Encoder\+Driver.\+py}} from its origional state, we left it as is, and created a single copy that would only be used for the R motor and deleted all information related to the Theta motor.\hypertarget{index_sec_lb4}{}\doxysection{Motor\+Driver.\+py}\label{index_sec_lb4}
...
The FSM for the Motor Tasks (including \mbox{\hyperlink{EncoderDriver_8py}{Encoder\+Driver.\+py}}, Control Loops, and \mbox{\hyperlink{MotorDriver_8py}{Motor\+Driver.\+py}}) is shown below. \hypertarget{index_sec_lb6}{}\doxysection{Servo\+Motor\+F.\+py}\label{index_sec_lb6}
...
We then use the file \mbox{\hyperlink{GcodeInterpreter_8py}{Gcode\+Interpreter.\+py}}, to take each line of GCODE, pull out the desired X-\/Y coordinates, and then preform some math to convert these to R-\/\+Theta coordinates. From these R-\/\+Theta coordinates, we use our known gear ratios to write the desired R Motor angle and Theta Motor angle to their own .txt files.

We then read each line of these files in \mbox{\hyperlink{main_8py}{main.\+py}} to get the setpoint of both motors. This is only done when the motor has reached its previous setpoint and the buffer variable is set to 1.
