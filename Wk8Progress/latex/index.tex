...
The files that we are using are\+: main Main file that runs all of our tasks and instantiates all objects Encoder\+Driver Read the position of the Theta Motor Encoder Read the position of the R Motor Motor\+Driver Control the motor by supplying a duty Theta\+Motor\+Control Closed loop control of Theta Motor RMotor\+Control Closed loop control of R Motor Servo\+MotorF Control the servo motor responsible for our half axis as well as limit switch reading Gcode\+Interpreter Writed R and Theta Setpoints to .txt file to be read later
